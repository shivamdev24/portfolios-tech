      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.success && data.data && Array.isArray(data.data)) {
            setUserDetails(data.data); // Store multiple users
          } else {
        ...
        <div className="mt-8">
          ...
          <div className="mb-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 w-full gap-10">
            {userDetails.map((user, index) => (
              <div key={index} className="p-4 border w-full h-fit rounded-lg shadow-sm bg-white">
                <div>
                {user.image_url ? (
                  <Image
                    src={user.image_url}
                ...
                <h3 className="text-lg font-semibold">{user.name || "not Updated"}</h3>
                <h5 className="text-base font-semibold">{user.email}</h5>
                <p className="text-gray-700"><strong>Availability:</strong> {user.availabilityStatus}</p>
                <p className="text-gray-700"><strong>Experience Level:</strong> {user.preferences.experienceLevel}</p>
                <p className="text-gray-700"><strong>Job Type:</strong> {user.preferences.jobType}</p>
                <p className="text-gray-700"><strong>Portfolio:</strong> <a href={`/portfolio/${user.username}`}>{user.username}</a></p>
